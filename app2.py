...
def _fetch_yfinance_data(all_tickers_tuple, start_date, end_date):
    ...
    try:
        ...
        # yfinance 최신버전 MultiIndex/SingleIndex 처리 보완
...
def fetch_market_data(tickers, start_date, end_date, force_refresh=False):
    """
    force_refresh=True 일 경우 캐시를 모두 비우고 새로운 시세를 다시 수집합니다.
    """
...
# -----------------------------------------------------------------------------
# 3. Streamlit 대시보드 메인 설정 및 메뉴 선언
# -----------------------------------------------------------------------------
...
if menu == "트렌드 리포트":
    ...
    if pf_df.empty:
        ...
        with st.form("trend_control_form"):
            ...
            with f_col1:
                ...
                selected_bm = st.multiselect(
                    ...
                    help="선택한 벤치마크 지수의 수익률 트렌드가 수익률 그래프에 점선으로 표시됩니다."
                )
            ...
            with f_col3:
                mirae_eval_val = st.number_input(
                    ...
                    help="미래에셋 증권 계좌는 수동 입력 가액이 전 분석 기간에 공통 반영됩니다."
                )
        ...
        if run_button:
            ...
            with st.spinner("최신 시세 및 벤치마크 데이터를 다시 수집하고 트렌드를 계산 중입니다..."):
        ...
        if 'trend_calc_df' in st.session_state and not st.session_state['trend_calc_df'].empty:
            ...
            with disp_col1:
                layout_setting = st.selectbox(
                    "🖥️ 차트 Layout 선택",
                    options=["1열 (기존 세로 배치)", "2열 (좌우 2개 분할)", "3열 (좌우 3개 분할)"],
                    index=0, key="live_chart_layout"
                )
            ...
            with disp_col3:
                y_range_mode = st.radio(
                    ...
                    horizontal=True, key="live_y_range_mode"
                )
            ...
            with disp_col4:
                if y_range_mode == "수동 지정 (Manual)":
                    ...
                    with r_c1:
                        y_min_val = st.number_input("Y축 최소값 (원)", value=0.0, step=1000000.0, key="live_ymin")
                    with r_c2:
                        y_max_val = st.number_input("Y축 최대값 (원)", value=100000000.0, step=1000000.0, key="live_ymax")

            num_cols = 1
            if "2열" in layout_setting: num_cols = 2
            elif "3열" in layout_setting: num_cols = 3

            y_scale_setting = "log" if y_scale_choice == "Logarithmic (로그)" else "linear"

            def apply_y_axis_config(fig, axis_name="yaxis", is_money=True):
                kwargs = dict(type=y_scale_setting, zeroline=True)
                if is_money and y_range_mode == "수동 지정 (Manual)" and y_min_val is not None and y_max_val is not None:
            ...
            def draw_single_chart(sub_df, title_name, force_single_col=False):
                ...
                sub_df['구간별 누적수익률'] = np.where(initial_eval > 0, ((sub_df['총평가금액'] - initial_eval) / initial_eval) * 100, 0)
                ...
                fig1.add_trace(go.Bar(x=sub_df['Chart_Date'], y=sub_df['평가손익'], name="전체 누적 평가손익", marker_color='#e05d5d', opacity=0.5), secondary_y=False)
                fig1.add_trace(go.Scatter(
                    x=sub_df['Chart_Date'], y=sub_df['총평가금액'], name="총평가금액",
                    mode='lines+markers+text', line=dict(color='#ff9900', width=3), marker=dict(size=6),
                    text=[f"{v:,.0f}" for v in sub_df['총평가금액']], textposition="top center"
                ), secondary_y=False)
                fig1.add_trace(go.Scatter(
                    x=sub_df['Chart_Date'], y=sub_df['수익률'], name="수익률(%)",
                    mode='lines+markers', line=dict(color='#2ca02c', dash='dash', width=2),
                    marker=dict(size=6), hovertemplate="%{y:.2f}%"
                ), secondary_y=True)

                fig1.update_layout(title=f"1. [{title_name}] 자산 및 전체 손익/수익률 추이", barmode='relative', hovermode="x unified", height=430)
                fig1.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                apply_y_axis_config(fig1, axis_name="yaxis", is_money=True)
                fig1.update_yaxes(title_text="금액 (원)", tickformat=",.0f", secondary_y=False)
                fig1.update_yaxes(title_text="수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True, secondary_y=True)

                fig2 = go.Figure()
                valid_period_df = sub_df.dropna(subset=['주기별 평가손익'])
                period_colors = ['#2ca02c' if v >= 0 else '#d62728' for v in valid_period_df['주기별 평가손익']]
                fig2.add_trace(go.Bar(x=valid_period_df['Chart_Date'], y=valid_period_df['주기별 평가손익'], name="주기별 평가손익", marker_color=period_colors, opacity=0.85))
                fig2.add_trace(go.Scatter(x=sub_df['Chart_Date'], y=sub_df['선택구간 누적손익'], name="선택구간 누적손익 (추이)", mode='lines+markers', line=dict(color='#9467bd', width=2.5), marker=dict(size=5)))

                fig2.update_layout(title=f"2. [{title_name}] 구간 손익 금액 추이", hovermode="x unified", height=430)
                fig2.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                apply_y_axis_config(fig2, axis_name="yaxis", is_money=True)
                fig2.update_yaxes(title_text="손익금액 (원)", tickformat=",.0f")

                fig3a = go.Figure()
                fig3a.add_trace(go.Scatter(x=sub_df['Chart_Date'], y=sub_df['구간별 누적수익률'], name="구간별 누적수익률 (%)", mode='lines+markers', line=dict(color='#1f77b4', width=2.5), marker=dict(size=5), hovertemplate="%{y:.2f}%"))

                for bm_name, bm_df in bm_calc_dict.items():
                    bm_df['dt_temp'] = pd.to_datetime(bm_df['Date_str'])
                    bm_df = bm_df.sort_values('dt_temp', ascending=True).reset_index(drop=True)
                    bm_df['Chart_Date'] = bm_df['dt_temp'].dt.strftime('%Y-%m-%d')
                    bm_df.drop(columns=['dt_temp'], inplace=True)

                    fig3a.add_trace(go.Scatter(
                        x=bm_df['Chart_Date'], y=bm_df['기간 누적 수익률'],
                        mode='lines', name=f"📌 {bm_name} 누적수익률 (%)",
                        line=dict(color=bm_styles.get(bm_name, {}).get('color', '#7f7f7f'), dash='dot'),
                        hovertemplate="%{y:.2f}%"
                    ))

                fig3a.update_layout(title=f"3-1. [{title_name}] 구간 누적수익률 추이 (벤치마크 비교)", hovermode="x unified", height=430)
                fig3a.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                fig3a.update_yaxes(title_text="수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True)

                fig3b = go.Figure()
                fig3b.add_trace(go.Scatter(x=sub_df['Chart_Date'], y=sub_df['구간별 수익률'], name="구간별 수익률 (%)", mode='lines+markers', line=dict(color='#17becf', width=2, dash='dot'), marker=dict(size=5), hovertemplate="%{y:.2f}%"))
                ...
                for bm_name, bm_df in bm_calc_dict.items():
                    ...
                    fig3b.add_trace(go.Scatter(
                        x=bm_df['Chart_Date'], y=bm_df['주기별 수익률'],
                        mode='lines', name=f"📌 {bm_name} 주기별수익률 (%)",
                        line=bm_styles.get(bm_name, dict(dash='dash')),
                        hovertemplate="%{y:.2f}%"
                    ))

                fig3b.update_layout(title=f"3-2. [{title_name}] 주기별 수익률 추이 (벤치마크 비교)", hovermode="x unified", height=430)
                fig3b.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                fig3b.update_yaxes(title_text="수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True)
                ...
                if effective_cols == 1:
                    ...
                    st.plotly_chart(fig2, use_container_width=True, key=f"trend_fig2_{title_name}")
                    st.plotly_chart(fig3a, use_container_width=True, key=f"trend_fig3a_{title_name}")
                    st.plotly_chart(fig3b, use_container_width=True, key=f"trend_fig3b_{title_name}")
                elif effective_cols == 2:
                    col_a, col_b = st.columns(2)
                    with col_a: st.plotly_chart(fig1, use_container_width=True, key=f"trend_fig1_{title_name}")
                    with col_b: st.plotly_chart(fig2, use_container_width=True, key=f"trend_fig2_{title_name}")
                    col_c, col_d = st.columns(2)
                    with col_c: st.plotly_chart(fig3a, use_container_width=True, key=f"trend_fig3a_{title_name}")
                    with col_d: st.plotly_chart(fig3b, use_container_width=True, key=f"trend_fig3b_{title_name}")
                else:
            ...
            def draw_group_summary_charts(df, group_col, prefix):
                ...
                grp_agg = df.groupby(['Date', group_col])[['원금', '평가손익', '총평가금액']].sum().reset_index()
                ...
                grp_agg['수익률'] = np.where(grp_agg['원금'] > 0, (grp_agg['평가손익'] / grp_agg['원금']) * 100, 0)
                ...
                grp_agg['주기별 수익률'] = grp_agg.groupby(group_col)['총평가금액'].pct_change() * 100
                first_eval = grp_agg.groupby(group_col)['총평가금액'].transform('first')
                grp_agg['선택기간 누적 수익률'] = np.where(first_eval > 0, ((grp_agg['총평가금액'] - first_eval) / first_eval) * 100, 0)
                ...
                for grp in groups:
                    ...
                    fig_sel_p.add_trace(go.Scatter(x=sub['Chart_Date'], y=sub['선택구간 누적손익'], name=f"[누적손익] {grp}", mode='lines+markers', hovertemplate="%{y:,.0f} 원"))

                fig_sel_p.update_layout(title=f"🔹 [{prefix}] 선택 구간 누적 평가손익 Trend", hovermode="x unified", height=450)
                fig_sel_p.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                ...
                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp].dropna(subset=['주기별 평가손익'])
                    fig_period_p.add_trace(go.Bar(x=sub['Chart_Date'], y=sub['주기별 평가손익'], name=str(grp)))
                fig_period_p.update_layout(title=f"🔹 [{prefix}] 선택 기간 주기별 평가손익 Trend (세로 누적 막대)", barmode='relative', hovermode="x unified", height=430)
                fig_period_p.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                ...
                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp].dropna(subset=['주기별 수익률'])
                    fig_period_ret.add_trace(go.Scatter(x=sub['Chart_Date'], y=sub['주기별 수익률'], name=str(grp), mode='lines+markers', hovertemplate="%{y:.2f}%"))
                ...
                for bm_name, bm_df in bm_calc_dict.items():
                    ...
                    fig_period_ret.add_trace(go.Scatter(x=sub_bm['Chart_Date'], y=sub_bm['주기별 수익률'], mode='lines', name=f"📌 {bm_name}", line=bm_styles.get(bm_name, dict(dash='dot')), hovertemplate="%{y:.2f}%"))

                fig_period_ret.update_layout(title=f"🔹 [{prefix}] 선택기간 주기별 수익률 Trend (꺾은선, 벤치마크 포함)", hovermode="x unified", height=430)
                fig_period_ret.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                fig_period_ret.update_yaxes(title_text="주기별 수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True)

                fig_cum_ret = make_subplots(specs=[[{"secondary_y": True}]])
                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp]
                    fig_cum_ret.add_trace(go.Bar(x=sub['Chart_Date'], y=sub['선택구간 누적손익'], name=f"[누적평가손익] {grp}", opacity=0.7, hovertemplate="%{y:,.0f} 원"), secondary_y=True)

                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp]
                    fig_cum_ret.add_trace(go.Scatter(x=sub['Chart_Date'], y=sub['선택기간 누적 수익률'], name=f"[누적수익률] {grp}", mode='lines+markers', hovertemplate="%{y:.2f}%"), secondary_y=False)
                ...
                for bm_name, bm_df in bm_calc_dict.items():
                    ...
                    bm_df['Chart_Date'] = bm_df['dt_temp'].dt.strftime('%Y-%m-%d')
                    bm_df.drop(columns=['dt_temp'], inplace=True)
                    fig_cum_ret.add_trace(go.Scatter(x=bm_df['Chart_Date'], y=bm_df['기간 누적 수익률'], mode='lines', name=f"📌 {bm_name}", line=bm_styles.get(bm_name, dict(dash='dot')), hovertemplate="%{y:.2f}%"), secondary_y=False)

                fig_cum_ret.update_layout(title=f"🔹 [{prefix}] 선택기간 누적 수익률 Trend (좌측) & 선택기간 누적평가 손익 (우측 보조축 그룹 막대)", barmode='group', hovermode="x unified", height=450)
                fig_cum_ret.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                apply_y_axis_config(fig_cum_ret, axis_name="yaxis", is_money=False)
                fig_cum_ret.update_yaxes(title_text="누적 수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True, secondary_y=False)
                fig_cum_ret.update_yaxes(title_text="선택기간 누적평가 손익 (원)", tickformat=",.0f", secondary_y=True)

                fig_p = go.Figure()
                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp]
                    fig_p.add_trace(go.Bar(x=sub['Chart_Date'], y=sub['평가손익'], name=str(grp)))
                fig_p.update_layout(title=f"🔹 [{prefix}] 전체 통산 누적 평가손익 Trend (세로 누적 막대)", barmode='relative', hovermode="x unified", height=430)
                fig_p.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                ...
                for grp in groups:
                    sub = grp_agg[grp_agg[group_col] == grp]
                    fig_r.add_trace(go.Scatter(x=sub['Chart_Date'], y=sub['수익률'], name=str(grp), mode='lines+markers', hovertemplate="%{y:.2f}%"))
                fig_r.update_layout(title=f"🔹 [{prefix}] 통산 수익률 Trend (원금대비 꺾은선)", hovermode="x unified", height=430)
                fig_r.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                fig_r.update_yaxes(title_text="수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True)

                if num_cols == 1:
                    st.plotly_chart(fig_sel_p, use_container_width=True, key=f"trend_grp_sel_p_{prefix}")
                    st.plotly_chart(fig_period_p, use_container_width=True, key=f"trend_grp_period_p_{prefix}")
                    st.plotly_chart(fig_period_ret, use_container_width=True, key=f"trend_grp_period_ret_{prefix}")
                    st.plotly_chart(fig_cum_ret, use_container_width=True, key=f"trend_grp_cum_ret_{prefix}")
                    st.plotly_chart(fig_p, use_container_width=True, key=f"trend_grp_p_{prefix}")
                    st.plotly_chart(fig_r, use_container_width=True, key=f"trend_grp_r_{prefix}")
                elif num_cols == 2:
                    col1, col2 = st.columns(2)
                    with col1: st.plotly_chart(fig_sel_p, use_container_width=True, key=f"trend_grp_sel_p_{prefix}")
                    with col2: st.plotly_chart(fig_period_p, use_container_width=True, key=f"trend_grp_period_p_{prefix}")
                    col3, col4 = st.columns(2)
                    with col3: st.plotly_chart(fig_period_ret, use_container_width=True, key=f"trend_grp_period_ret_{prefix}")
                    with col4: st.plotly_chart(fig_cum_ret, use_container_width=True, key=f"trend_grp_cum_ret_{prefix}")
                    col5, col6 = st.columns(2)
                    with col5: st.plotly_chart(fig_p, use_container_width=True, key=f"trend_grp_p_{prefix}")
                    with col6: st.plotly_chart(fig_r, use_container_width=True, key=f"trend_grp_r_{prefix}")
                else:
            ...
            def render_separate_charts(df, group_col, prefix):
                if group_col is None:
                    agg_df = df.groupby('Date')[['원금', '평가손익', '총평가금액']].sum().reset_index()
                    ...
                    if num_cols > 1 and group_col is not None:
                        ...
                        for idx, grp in enumerate(groups):
                            with cols[idx % num_cols]:
                                grp_df = df[df[group_col] == grp].groupby('Date')[['원금', '평가손익', '총평가금액']].sum().reset_index()
                                st.markdown(f"##### 🎯 {prefix}: {grp}")
                                draw_single_chart(grp_df, f"{prefix} [{grp}]", force_single_col=True)
                    else:
                        for grp in groups:
                            grp_df = df[df[group_col] == grp].groupby('Date')[['원금', '평가손익', '총평가금액']].sum().reset_index()
                            st.markdown(f"#### 📌 {prefix}: {grp}")
            ...
            for i, v_type in enumerate(active_views):
                with tabs[i]:
                    if v_type == "전체 합산": render_separate_charts(calc_df, None, "전체 합산")
                    elif v_type == "계좌별": render_separate_charts(calc_df, 'account_num', "계좌별")
                    elif v_type == "증권사(Broker)별": render_separate_charts(calc_df, 'broker', "증권사")
                    elif v_type == "계좌유형별": render_separate_charts(calc_df, 'account_type', "계좌유형")
...
# -----------------------------------------------------------------------------
    ...
    if pf_df.empty:
        ...
        freq_ret_options = ["일간 (매일)", "일간 (주말 제외)", "주간 (매주 토요일)", "월간 (매달 말일)"]
        bm_options = ["미국 SPY", "미국 QQQ", "한국 KOSPI"]
        view_options = ["전체 합산", "계좌별", "증권사(Broker)별", "계좌유형별", "Category 1별", "Category 4별"]
        ...
        with st.form("return_report_form"):
            ...
            with c1:
                ...
                view_types = st.multiselect(
                    ...
                    default=st.session_state.get('ret_view_types', ["전체 합산", "계좌별", "증권사(Broker)별", "계좌유형별"])
                )
                selected_bm = st.multiselect(
                    "📈 비교 벤치마크 지수 선택 (차트에 함께 표시)",
                    options=bm_options,
                    default=st.session_state.get('ret_sel_bm', bm_options),
                    help="선택한 벤치마크 지수의 수익률 트렌드가 차트에 점선으로 그려집니다."
                )

            with c2:
                saved_ret_freq = st.session_state.get('ret_freq_str', "주간 (매주 토요일)")
                ...
                d1, d2 = st.columns(2)
                with d1:
                    start_date = st.date_input("조회 시작일", st.session_state.get('ret_start_date', min_rec_date))
                with d2:
                    end_date = st.date_input("조회 종료일", st.session_state.get('ret_end_date', max_rec_date if max_rec_date >= min_rec_date else min_rec_date))

            st.write("---")
            mirae_val = st.number_input(
                "🏦 미래에셋 계좌 평가금액 수동 입력 (원)", 
                value=st.session_state.get('ret_mirae_val', 0.0), step=1000000.0
            )
        ...
        if run_returns:
            ...
            filtered_pf_df = pf_df[pf_df['account_num'].isin(selected_accounts)].copy()
            fetch_tickers = list(filtered_pf_df['ticker'].unique()) + list(bm_ticker_map.values())

            with st.spinner("최신 시세 및 벤치마크 데이터를 다시 수집하고 수익률을 산출 중입니다..."):
                s_str = start_date.strftime('%Y-%m-%d')
                e_str = (end_date + pd.Timedelta(days=3)).strftime('%Y-%m-%d')
                market_data = fetch_market_data(fetch_tickers, s_str, e_str, force_refresh=True)
                    ...
                    if "미래에셋" in str(broker_name):
                        ...
                        if not acc_pf.empty:
                            ...
                            for _, row in current_pf.iterrows():
                                ...
                                cat1 = row['category1'] if pd.notna(row['category1']) and str(row['category1']).strip() != '' else '미지정'
                                cat4 = row['category4'] if pd.notna(row['category4']) and str(row['category4']).strip() != '' else '미지정'
        ...
        if 'returns_calc_df' in st.session_state and not st.session_state['returns_calc_df'].empty:
            raw_returns_df = st.session_state['returns_calc_df']
            bm_calc_dict = st.session_state.get('returns_bm_calc', {})
            active_ret_views = st.session_state.get('ret_view_types', ["전체 합산", "계좌별", "증권사(Broker)별", "계좌유형별"])
            ...
            with disp_col1:
                layout_setting_ret = st.selectbox(
                    "🖥️ 차트 Layout 선택",
                    options=["1열 (기존 세로 배치)", "2열 (좌우 2개 분할)", "3열 (좌우 3개 분할)"],
                    index=0, key="ret_live_chart_layout"
                )
            ...
            with disp_col3:
                y_range_mode_ret = st.radio(
                    "🎯 Y축 Range(범위) 지정",
                    options=["자동 (Auto)", "수동 지정 (Manual)"],
                    horizontal=True, key="ret_live_y_range_mode"
                )
            ...
            with disp_col4:
                if y_range_mode_ret == "수동 지정 (Manual)":
                    r_c1, r_c2 = st.columns(2)
                    with r_c1:
                        y_min_val_ret = st.number_input("수익률 Y축 최소값 (%)", value=-50.0, step=5.0, key="ret_live_ymin")
                    with r_c2:
                        y_max_val_ret = st.number_input("수익률 Y축 최대값 (%)", value=100.0, step=10.0, key="ret_live_ymax")

            num_cols_ret = 1
            if "2열" in layout_setting_ret: num_cols_ret = 2
            elif "3열" in layout_setting_ret: num_cols_ret = 3

            y_scale_setting_ret = "log" if y_scale_choice_ret == "Logarithmic (로그)" else "linear"

            def apply_y_axis_config_ret(fig, axis_name="yaxis"):
                kwargs = dict(type=y_scale_setting_ret, zeroline=True)
                if y_range_mode_ret == "수동 지정 (Manual)" and y_min_val_ret is not None and y_max_val_ret is not None:
            ...
            def calculate_group_returns(df, group_col=None):
                if group_col is None:
                    grp_df = df.groupby(['Date', 'Date_str'])['평가금액'].sum().reset_index().sort_values('Date')
                    grp_df['Group'] = "전체 합산"
                else:
                    grp_df = df.groupby(['Date', 'Date_str', group_col])['평가금액'].sum().reset_index().sort_values(['Date'])
                ...
                for grp_name, group_data in grp_df.groupby('Group'):
                    ...
                    initial_base_eval = group_data.loc[0, '평가금액'] if len(group_data) > 0 else 0
                    group_data['주기별 수익률'] = np.nan
                    group_data['기간 누적 수익률'] = np.nan

                    for i in range(len(group_data)):
                        curr_eval = group_data.loc[i, '평가금액']
                        prev_eval = group_data.loc[i-1, '평가금액'] if i > 0 else initial_base_eval

                        if i > 0 and prev_eval > 0:
                            group_data.loc[i, '주기별 수익률'] = ((curr_eval - prev_eval) / prev_eval) * 100
                        if initial_base_eval > 0:
                            group_data.loc[i, '기간 누적 수익률'] = ((curr_eval - initial_base_eval) / initial_base_eval) * 100
                ...
                fig.update_layout(
                    updatemenus=[
                        dict(
                            ...
                            showactive=False,
                            buttons=[
                                dict(label="✅ 모두 표시", method="restyle", args=["visible", True]),
                                dict(label="❌ 모두 숨기기", method="restyle", args=["visible", "legendonly"])
                            ]
                        )
                    ]
                )
            ...
            def render_returns_view(df, group_col, prefix):
                ...
                if calc_res.empty: return

                calc_res['dt_temp'] = pd.to_datetime(calc_res['Date_str'])
                calc_res = calc_res.sort_values('dt_temp', ascending=True).reset_index(drop=True)
                ...
                if group_col is not None and len(groups) > 1:
                    ...
                    for grp in groups:
                        ...
                        fig_period_comp.add_trace(
                            go.Bar(
                                x=sub['Chart_Date'], y=sub['평가금액'],
                                name=f"[평가금액] {grp}", opacity=0.35,
                                hovertemplate="%{y:,.0f} 원"
                            ), secondary_y=True
                        )

                    for grp in groups:
                        sub = calc_res[calc_res['Group'] == grp].dropna(subset=['주기별 수익률'])
                        fig_period_comp.add_trace(
                            go.Scatter(
                                x=sub['Chart_Date'], y=sub['주기별 수익률'],
                                mode='lines+markers', name=f"[수익률] {grp}",
                                hovertemplate="%{y:.2f}%"
                            ), secondary_y=False
                        )
                    ...
                    for bm_name, bm_df in bm_calc_dict.items():
                        ...
                        fig_period_comp.add_trace(
                            go.Scatter(
                                x=sub_bm['Chart_Date'], y=sub_bm['주기별 수익률'],
                                mode='lines', name=f"📌 {bm_name}",
                    ...
                    fig_period_comp.update_layout(
                        title=f"🔹 [{prefix}] 선택 기간 주기별 수익률 (좌축) & 평가금액 그룹 막대 (우측 보조축)",
                        barmode='group', hovermode="x unified", height=520
                    )
                    fig_period_comp.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                    fig_period_comp.update_yaxes(title_text="주기별 수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True, secondary_y=False)
                    fig_period_comp.update_yaxes(title_text="평가금액 (원)", tickformat=",.0f", secondary_y=True)
                    ...
                    for grp in groups:
                        ...
                        fig_cum_comp.add_trace(
                            go.Scatter(
                                x=sub['Chart_Date'], y=sub['기간 누적 수익률'],
                                mode='lines+markers', name=f"[수익률] {grp}",
                                hovertemplate="%{y:.2f}%"
                            ), secondary_y=False
                        )
                    ...
                    fig_cum_comp.update_layout(
                        title=f"🔹 [{prefix}] 선택 기간 누적 수익률 (좌축) & 평가금액 그룹 막대 (우측 보조축)",
                        barmode='group', hovermode="x unified", height=520
                    )
                    fig_cum_comp.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                    fig_cum_comp.update_yaxes(title_text="누적 수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True, secondary_y=False)
                    fig_cum_comp.update_yaxes(title_text="평가금액 (원)", tickformat=",.0f", secondary_y=True)
                    apply_y_axis_config_ret(fig_cum_comp, axis_name="yaxis")

                    if num_cols_ret == 1:
                        st.plotly_chart(fig_period_comp, use_container_width=True, key=f"ret_period_comp_{prefix}")
                        st.plotly_chart(fig_cum_comp, use_container_width=True, key=f"ret_cum_comp_{prefix}")
                    else:
                        r_col1, r_col2 = st.columns(2)
                        with r_col1: st.plotly_chart(fig_period_comp, use_container_width=True, key=f"ret_period_comp_{prefix}")
                        with r_col2: st.plotly_chart(fig_cum_comp, use_container_width=True, key=f"ret_cum_comp_{prefix}")
                    st.write("---")

                def render_single_group_ret(grp):
                    grp_data = calc_res[calc_res['Group'] == grp].sort_values('Date').reset_index(drop=True)
                    st.markdown(f"#### 📌 {prefix}: {grp}")
                    ...
                    fig.add_trace(
                        go.Bar(
                            x=grp_data['Chart_Date'], y=grp_data['평가금액'],
                            name="평가금액", marker_color='#2b5c8f', opacity=0.3,
                            hovertemplate="%{y:,.0f} 원"
                        ), secondary_y=True
                    )
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=grp_data['Chart_Date'], y=grp_data['주기별 수익률'],
                            mode='lines+markers+text', name='주기별 수익률 (%)',
                            line=dict(color='#1f77b4', width=2),
                            text=[f"{v:.2f}%" if pd.notna(v) else "" for v in grp_data['주기별 수익률']],
                            textposition="top center"
                        ), secondary_y=False
                    )
                    fig.add_trace(
                        go.Scatter(
                            x=grp_data['Chart_Date'], y=grp_data['기간 누적 수익률'],
                            mode='lines+markers+text', name='누적 수익률 (%)',
                            line=dict(color='#ff7f0e', width=3),
                            text=[f"{v:.2f}%" if pd.notna(v) else "" for v in grp_data['기간 누적 수익률']],
                    ...
                    fig.update_xaxes(type='category', categoryorder='array', categoryarray=date_order_list)
                    fig.update_yaxes(title_text="수익률 (%)", tickformat=",.2f", ticksuffix="%", zeroline=True, secondary_y=False)
                    fig.update_yaxes(title_text="평가금액 (원)", tickformat=",.0f", secondary_y=True)
                    apply_y_axis_config_ret(fig, axis_name="yaxis")
                    st.plotly_chart(fig, use_container_width=True, key=f"ret_grp_{prefix}_{grp}")

                    st.subheader(f"📋 [{grp}] 상세 수익률 데이터")
                    disp_table = grp_data[['Date_str', '평가금액', '주기별 수익률', '기간 누적 수익률']].copy()
                    disp_table.rename(columns={'Date_str': '날짜'}, inplace=True)
                    disp_table['평가금액'] = disp_table['평가금액'].map('{:,.0f} 원'.format)
                    disp_table['주기별 수익률'] = disp_table['주기별 수익률'].map(lambda x: f"{x:.2f}%" if pd.notna(x) else "n/a")
                    disp_table['기간 누적 수익률'] = disp_table['기간 누적 수익률'].map(lambda x: f"{x:.2f}%" if pd.notna(x) else "n/a")
            ...
            for i, v_type in enumerate(active_ret_views):
                with tabs[i]:
                    if v_type == "전체 합산": render_returns_view(raw_returns_df, None, "전체 합산")
                    elif v_type == "계좌별": render_returns_view(raw_returns_df, 'account_num', "계좌별")
                    elif v_type == "증권사(Broker)별": render_returns_view(raw_returns_df, 'broker', "증권사")
                    elif v_type == "계좌유형별": render_returns_view(raw_returns_df, 'account_type', "계좌유형")
                    elif v_type == "Category 1별": render_returns_view(raw_returns_df, 'category1', "Category 1")
                    elif v_type == "Category 4별": render_returns_view(raw_returns_df, 'category4', "Category 4")
...
# -----------------------------------------------------------------------------
    ...
    st.write("각 계좌번호에 수동으로 직관적인 별칭을 입력하거나 변경할 수 있습니다.")
    ...
    if pf_df.empty:
        st.warning("등록된 계좌 정보가 없습니다. [포트폴리오 업로드]를 먼저 진행해 주세요.")
    else:
        merged_df = pd.merge(pf_df, alias_df, on='account_num', how='left').fillna({'alias': ''})
        ...
        with st.form("alias_form"):
            ...
            for idx, row in merged_df.iterrows():
                ...
                with col3:
                    new_alias = st.text_input(f"별칭 ({row['account_num']})", value=row['alias'], key=f"alias_{row['account_num']}")
...
# -----------------------------------------------------------------------------
    ...
    with tab2:
        ...
        with st.form("cash_flow_form"):
            ...
            with col2:
                init_accs = pd.read_sql("SELECT account_num FROM initial_principal", conn)['account_num'].tolist()
            ...
            if submit_cf and acc_choice != "등록된 계좌 없음" and amount > 0:
                ...
                c.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (trans_date.strftime('%Y-%m-%d'), acc_choice, flow_type, amount, note))
                conn.commit()
