        ...
        with st.form("return_report_form"):
            ...
            with c1:
                ...
                selected_bm = st.multiselect(
                    ...
                    help="선택한 벤치마크 지수(SPY, QQQ, KOSPI)의 수익률 트렌드가 차트에 점선으로 그려집니다."
                )
        ...
        if 'returns_calc_df' in st.session_state and not st.session_state['returns_calc_df'].empty:
            ...
            with disp_col1:
                layout_setting_ret = st.selectbox(
                    "🖥️ 차트 Layout 선택",
                    options=["1열 (기존 세로 배치)", "2열 (좌우 2개 분할)", "3열 (좌우 3개 분할)"],
                    index=1, key="ret_live_chart_layout"
                )
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                ...
                if st.button("DB에 포트폴리오 저장 (해당 기준일자 데이터만 업데이트)"):
                    conn = get_connection()
                    c = conn.cursor()
